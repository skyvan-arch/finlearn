...
interface ChallengeQuestion {
  ...
  chapterTitle: string; chapterSlug: string; bookTitle: string; bookSlug: string; bookColor: string
}

type Phase = "answering" | "revealed" | "done"

const ACCENT = "#6366f1" // indigo — neutral across all books
...
export default function ChallengeEngine() {
  ...
  const answersRef = useRef<{ isCorrect: boolean }[]>([])
  ...
  if (loading) return <div className="py-8 text-center" style={{ color: "var(--c-muted)" }}>Loading challenge…</div>
  if (questions.length === 0) return <div className="py-8 text-center" style={{ color: "var(--c-muted)" }}>No questions available yet.</div>
    ...
    return (
      <div className="text-center py-8">
        ...
        <div className="space-y-3">
          <button onClick={reset} className="w-full py-3 rounded-lg font-semibold text-white" style={{ backgroundColor: ACCENT }}>
            New Challenge
          </button>
  ...
  function next() {
    ...
    if (isLast) {
      setFinalScore(answersRef.current.filter((a) => a.isCorrect).length)
      setPhase("done")
    } else {
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ backgroundColor: "var(--c-border)" }}>
          <div className="h-full rounded-full transition-all duration-300" style={{ width: `${(current / questions.length) * 100}%`, backgroundColor: ACCENT }} />
        </div>
      ...
      {phase === "revealed" && (
        <button onClick={next} className="w-full mt-4 py-3 rounded-lg font-semibold text-white text-sm" style={{ backgroundColor: ACCENT }}>
